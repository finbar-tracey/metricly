...
struct ExerciseSubstitutionsView: View {
    ...
    private var substitutions: [SubstitutionGroup] {
        ...
        // If no exact match, try muscle group matching
        if matches.isEmpty {
            for group in Self.substitutionDatabase {
                if name.contains(group.muscleGroup.lowercased()) {
                    matches.append(group)
                }
            }
        }
        ...
        List {
            if substitutions.isEmpty {
                ...
                ForEach(substitutions) { group in
                    Section {
                        ForEach(group.exercises.filter { $0.lowercased() != exerciseName.lowercased() }, id: \.self) { exercise in
                            HStack {
                        ...
                        Label(group.muscleGroup, systemImage: group.icon)
                    } footer: {
                        Text(group.notes)
                            .font(.caption2)
                    }
...
#Preview {
    NavigationStack {
        ExerciseSubstitutionsView(exerciseName: "Bench Press")
    }
}
