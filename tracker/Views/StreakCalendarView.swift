...
struct StreakCalendarView: View {
    ...
    private var currentStreak: Int {
        let dates = Set(workouts.map { cal.startOfDay(for: $0.date) })
        var streak = 0
    ...
    private var longestStreak: Int {
        let dates = workouts.map { cal.startOfDay(for: $0.date) }
    ...
    private var heroCard: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                HStack(spacing: 0) {
                    heroStatCol("This Week", value: "\(thisWeekCount)")
                    Rectangle().fill(.white.opacity(0.25)).frame(width: 1, height: 28)
                    heroStatCol("This Month", value: "\(thisMonthCount)")
                    Rectangle().fill(.white.opacity(0.25)).frame(width: 1, height: 28)
                    heroStatCol("Total", value: "\(totalWorkouts)")
                }
            }
            .padding(20)
        }
                ...
                ForEach(Array(weeks.enumerated()), id: \.offset) { idx, week in
                    VStack(spacing: 3) {
                        ForEach(week, id: \.self) { day in
                            let count = workoutDates[cal.startOfDay(for: day)] ?? 0
