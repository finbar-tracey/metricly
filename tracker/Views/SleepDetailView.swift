...
struct SleepDetailView: View {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                HStack(spacing: 0) {
                    if let inBed = todaySleep.inBed {
                        sleepStatColumn(icon: "bed.double.fill", label: "Bedtime",
                                        value: inBed.formatted(.dateTime.hour().minute()))
                    }
                    if let wake = todaySleep.wakeUp {
                        sleepStatColumn(icon: "sun.horizon.fill", label: "Wake",
                                        value: wake.formatted(.dateTime.hour().minute()))
                    }
                    if let eff = sleepEfficiency {
                        sleepStatColumn(icon: "gauge.with.needle.fill", label: "Efficiency",
                                        value: "\(Int(eff))%")
                    }
                }
            }
                ...
                ForEach(TimeRange.allCases, id: \.self) { range in
                    Button {
                        ...
                        Text(range.rawValue)
                            ...
                            .background(timeRange == range ? AnyShapeStyle(Color.indigo) : AnyShapeStyle(Color(.secondarySystemFill)),
                                        in: Capsule())
                        ...
                        AxisMarks(values: .stride(by: 60)) { value in
                            ...
                            AxisValueLabel {
                                if let mins = value.as(Double.self) {
                                    Text(formatShiftedMinutes(mins)).font(.caption2)
                                }
                            }
                        }
                    }
    ...
    private var chartSleep: [(date: Date, minutes: Double)] { Array(dailySleep.suffix(dayCount)) }
