...
struct ExportHelper {
    static func generateCSV(workouts: [Workout]) -> String {
        var csv = "Date,Workout,Rating,Duration (min),Exercise,Muscle Group,Superset Group,Set,Reps,Weight (kg)\n"
        for workout in workouts.sorted(by: { $0.date > $1.date }) {
            ...
            for exercise in workout.exercises.sorted(by: { $0.order < $1.order }) {
                ...
                for (index, set) in exercise.sets.enumerated() {
                    let line = "\(dateStr),\(escape(workout.name)),\(ratingStr),\(durationMin),\(escape(exercise.name)),\(categoryStr),\(ssGroup),\(index + 1),\(set.reps),\(String(format: "%.1f", set.weight))\n"
                    csv += line
                }
            }
        }
