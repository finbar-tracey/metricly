...
struct Achievement: Identifiable {
    ...
    var isUnlocked: Bool
    var progress: Double? // 0-1 optional progress toward unlock
...
struct AchievementsView: View {
    ...
    private func filterChip(label: String, icon: String, color: Color, isSelected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .padding(.vertical, 6)
            .background(isSelected ? color.opacity(0.15) : Color(.systemGray6), in: Capsule())
    ...
    private var achievementListSection: some View {
        ...
        return ForEach(Achievement.Tier.allCases, id: \.self) { tier in
            let tierAchievements = filtered.filter { $0.tier == tier }
    ...
    private var gymAchievements: [Achievement] {
        ...
        // Early bird / night owl — workout time of day
        let earlyWorkouts = finishedWorkouts.filter {
            let hour = Calendar.current.component(.hour, from: $0.date)
            return hour >= 5 && hour < 8
        }.count
        list.append(Achievement(
            id: "early_bird", name: "Early Bird", description: "Complete 10 workouts before 8 AM",
    ...
    private var runningAchievements: [Achievement] {
        ...
        // From ExternalWorkout (HealthKit runs)
        let runs = externalWorkouts.filter { $0.workoutType == .running }
        let runCount = runs.count

        // From manual cardio sets with distance
        let manualRunSets = finishedWorkouts
            .flatMap { $0.exercises.filter { $0.category == .cardio } }
            .flatMap(\.sets)
            .filter { $0.isCardio && $0.distance != nil }
        let manualRunCount = manualRunSets.count

        let totalRunCount = runCount + manualRunCount

        // Total distance: external (meters → km) + manual (already km)
        ...
        list.append(Achievement(
            id: "first_run", name: "Off the Mark", description: "Complete your first run",
        ...
        // Distance milestones
        list.append(Achievement(
            id: "distance_10k", name: "First 10K", description: "Run a total of 10 km",
            icon: "map", tier: .bronze, category: .running,
            isUnlocked: totalKm >= 10, progress: min(1, totalKm / 10)
        ))
        list.append(Achievement(
            id: "distance_42k", name: "Marathon Distance", description: "Run a total of 42.2 km",
            icon: "medal", tier: .silver, category: .running,
            isUnlocked: totalKm >= 42.2, progress: min(1, totalKm / 42.2)
        ))
        list.append(Achievement(
            id: "distance_100k", name: "Ultra Runner", description: "Run a total of 100 km",
            icon: "medal.fill", tier: .gold, category: .running,
            isUnlocked: totalKm >= 100, progress: min(1, totalKm / 100)
        ))
        list.append(Achievement(
            id: "distance_500k", name: "Iron Legs", description: "Run a total of 500 km",
        ...
        list.append(Achievement(
            id: "single_5k", name: "5K Finisher", description: "Complete a single 5 km run",
            icon: "flag", tier: .bronze, category: .running,
            isUnlocked: longestRunKm >= 5, progress: min(1, longestRunKm / 5)
        ))
        list.append(Achievement(
            id: "single_10k", name: "10K Finisher", description: "Complete a single 10 km run",
            icon: "flag.fill", tier: .silver, category: .running,
            isUnlocked: longestRunKm >= 10, progress: min(1, longestRunKm / 10)
        ))
        list.append(Achievement(
            id: "single_half", name: "Half Marathon", description: "Complete a single 21.1 km run",
    ...
    private var stepsAchievements: [Achievement] {
        ...
        // Total volume
        list.append(Achievement(
            id: "steps_total_100k", name: "Hundred Thousand", description: "Walk 100,000 total steps",
            icon: "shoeprints.fill", tier: .bronze, category: .steps,
            isUnlocked: totalSteps >= 100_000, progress: min(1, totalSteps / 100_000)
        ))
        list.append(Achievement(
            id: "steps_total_1m", name: "Million Steps", description: "Walk 1,000,000 total steps",
        ...
        list.append(Achievement(
            id: "step_streak_7", name: "Step Streak", description: "7-day streak of 10K+ steps",
            icon: "flame", tier: .silver, category: .steps,
            isUnlocked: stepStreak >= 7, progress: min(1, Double(stepStreak) / 7)
        ))
        list.append(Achievement(
            id: "step_streak_30", name: "Step Legend", description: "30-day streak of 10K+ steps",
    ...
    private var sleepAchievements: [Achievement] {
        ...
        // Tracking
        list.append(Achievement(
            id: "sleep_tracked_30", name: "Sleep Tracker", description: "Track 30 nights of sleep",
    ...
    // MARK: - Steps Computed Properties
